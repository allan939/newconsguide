...
└── .github/
    └── workflows/
        ├── update.yml                  # Monthly stats rebuild (schedule + deals.json push + manual dispatch)
        └── auto-merge.yml              # Merges claude/** branches into main — merge only, nothing else
```

> **Important:** `index.html` and `sitemap.xml` are **generated artifacts**. Always edit `template.html`, `deals.json`, or `stats.json` — never edit `index.html`/`sitemap.xml` directly, they will be overwritten on the next `python3 build.py`.
